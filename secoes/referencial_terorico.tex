...
Os dados que servem como entrada do sistema \enquote{\textit{são a menor parte de uma informação}}, \cite{boscarioli2016mineracao}, e podem ser definidos como conhecimento bruto ou fatos isolados. Nesse estado, podemos tê-los como não adequadamente tratados para fornecer \textit{gnose} aos \textit{stakeholders}.
...
A não volatilidade do \textit{data warehouse} diz respeito às operações feitas nele. Enquanto um banco de dados comum tem por operações ordinárias a seleção, inclusão, atualização e exclusão, o \textit{data warehouse} executa somente a carga inicial e a consulta dos dados, isto significa que o dados não são alterados ou excluídos dentro dele o que o torna, nesse aspecto, mais simples que um banco de dados comum.
...
\begin{figure}[ht]
\centering
\includegraphics[width=.5\textwidth]{imagens/example-star-schema.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=.5\textwidth]{imagens/example-snowflake-schema.jpg}
