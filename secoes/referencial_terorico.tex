...
\begin{itemize}
  \item \textbf{Extração}: Consiste no processo de ler os dados de uma fonte de dados particular e extrair dele um subconjunto de dados.
  \item \textbf{Transformação}: Corresponde ao método de modificação/combinação dos dados extraídos previamente para adequá-los aos requisitos da nova base de dados.
  \item \textbf{Carga}: O processo de escrever os dados transformados anteriormente no base de dados objetivo (\textit{data darehouse}). \cite{goarETL}
\end{itemize} 
...
A extração é a primeira etapa do ETL, nela os dados serão extraídos das mais diversas fontes para uma área intermediária conhecida como \textit{staging area} ou área de preparação. Essa transferência de dados é feita para reduzir a carga sobre os sistemas que detêm as fontes de dados e também do próprio \textit{data warehouse}. Nesse ponto, os dados, sejam eles estruturados ou não estruturados, são importados em uma organização estável para um único local.

A partir da área de preparação